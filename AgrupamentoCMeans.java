...
{
    public static final double epsilonPadrao = 0.0000001; // criterio de parada
    ...
    public static final int maximo_iteracoes = 5000;
    ...
    public void aplicarAlgoritmo() 
    {
        ...
        while(!bConvergiu && iIteracao < maximo_iteracoes);
        System.out.println("Iterações realizadas: " + iIteracao);
    }
    ...
    public String recuperarResultado() 
    {
        if(matrizCentroides == null) {
            ...
            {
                buffer.append("Centróide Cluster " + i + ":\t");
    ...
    protected double[] centroide(double [][] matrizParticoes, int indiceCluster)
    {
        ...
        for(int i = 0; i < base.recuperarNumeroInstancias(); i++)
        {
            ...
            for(int j = 0; j < vetorCentroide.length; j++)
            {
                vetorCentroide[j] += base.recuperarInstancia(i)[j] * dPertinencia;
            }
        }        
    ...
    public double[][] calculaNovaMatrizParticoes()
    {
    	...
    	double[][] dMatrizCalculada = new double[base.recuperarNumeroInstancias()][nClusters];
    	int iQtInstanciasI = 0;
    	double dResultSomatorio = 0;
    	double dNumerador = 0;
    	double dResultParcial = 0;
    	
    	for (int k = 0; k < base.recuperarNumeroInstancias(); k++) 
        {
    		I = null;
    		iQtInstanciasI = 0;
    		for (int i = 0; i < nClusters; i++)
